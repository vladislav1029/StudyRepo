...
api = NinjaAPI()  # важно: csrf=True для сессий


@api.post("/login", auth=None)  # auth=None — доступно всем
...
def register_user(request, payload: RegisterSchema):
    if payload.password1 != payload.password2:
        return api.create_response(request, {"detail": "Passwords do not match"}, status=400)
